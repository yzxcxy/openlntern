...
- 下一轮再次进入压缩逻辑时，会先用最近一次摘要快照折叠已覆盖历史，再基于“摘要 + 有效上下文”重新判断是否还需要继续压缩。
...
- `InitEino(...)` 内部把 `context_compression` 转成运行时参数，并把 `summary_llm` 初始化成一个 `deepseek.ChatModel`，存进 `runtimeState.summaryModel`。

这里有一个实现细节需要特别记住：

- `summary_llm` 当前并不是只给“线程标题生成”使用。
- 同一个 `summaryModel` 同时被 `ensureThreadTitle(...)` 和 `summarizeContextHistory(...)` 复用。
- 当上下文压缩开启时，`summary_llm` 变成必填项；未配置会在启动阶段直接报错。
...
压缩逻辑现在不是直接拿“全量历史”做 `soft_limit` 判断，而是先构造本轮有效上下文：

- 如果线程上已经有摘要快照，会先按 `covered_until_msg_id` 折叠掉已覆盖历史。
- 然后把快照里的 `summary_text` 重新拼回一条 system 摘要消息。
- 再对“摘要 + 未覆盖历史 + 本轮注入上下文”做 token 估算。

在这个有效上下文之上，才会用一个非常粗粒度的估算器计算消息 token：

- 估算方式是 `len(role + content) / estimated_chars_per_token`
- 默认 `estimated_chars_per_token = 4`

如果有效上下文估算 token 总和没有超过 `soft_limit_tokens`，本轮直接跳过新的摘要压缩。

只有当“摘要 + 有效上下文”估算值仍然超过 `soft_limit_tokens` 时，`stats.Triggered` 才会变成 `true`，后续裁剪和摘要逻辑才会执行。

## 压缩执行阶段

### 先读取最近一次摘要快照

压缩开始后，服务会先调用 `loadLatestThreadContextSnapshot(threadID)` 取当前线程最近的一条快照，并把它当成下一轮上下文基线的一部分。
...
- 如果某条历史消息及其之前的区间已经被上一份摘要覆盖，那么这些非 system 历史会先从活动窗口中移除，不再参与本轮 `soft_limit` 判断。
- 这样可以避免出现“旧原文 + 旧摘要”同时进入 prompt 的重复堆积，也避免线程一旦超限后每轮都拿全量历史重复触发压缩。

### 保留规则

当前压缩器在“有效上下文仍然超限”时，会按一个比较明确的保留策略继续收缩窗口：

- 所有 `system` 消息默认固定保留。
- 最后一条 user 消息固定保留。
- 从尾部开始，额外保留最近 `max_recent_messages` 条消息。
- 裁剪目标不再只是压到 `hard_limit` 以下，而是优先给摘要本身留出空间，尽量把活动窗口压回 `soft_limit` 以下。

默认 `max_recent_messages = 14`。

### 二次裁剪

即使完成了“固定保留 + 最近窗口保留”，活动窗口仍然可能超过目标预算。这时还会继续从更早的可压缩消息里剥离内容进入摘要：

- 只从“最近窗口里保留的、但不是 pinned 的消息”中继续删除。
- 删除顺序是从更早的保留消息开始删。
- `system` 消息和最后一条 user 消息不会在这个阶段被删掉。

如果没有任何可继续压缩的消息了，但最终有效上下文仍然大于 `hard_limit_tokens`，函数会直接返回错误，整轮请求失败，不会用“兼容错误”的方式继续往下跑。
...
- 必须调用 `summary_llm` 输出 JSON，再做结构化清洗。
- 如果模型未配置、返回空内容、返回非法 JSON，都会直接报错，不再走 fallback 文本拼接。

还有一个很重要的实现行为：

- 只有本轮真的有新增被压缩消息时，才会再次调用 `summary_llm` 更新摘要。
- 如果只有旧摘要、没有新的压缩增量，不会重复调用摘要模型。
- 摘要生成或快照落库失败会直接中断本轮压缩，不再静默降级。
...
即使本轮没有新增被删消息，只要线程上已经存在 `summary_text`，压缩器也会把这份旧摘要直接作为下一轮活动上下文的一部分回填到 prompt。

不过摘要消息并不是必定插入，插入前还会再做一次预算判断：

- 先估算摘要 system 消息的 token
- 并优先围绕 `soft_limit` 预留摘要空间，必要时才退到 `hard_limit`
...
- 摘要生成失败、摘要模型未配置、摘要响应非法、摘要快照落库失败：直接中断请求。

这也是重构后的取舍：摘要不再是“可有可无的增强”，而是压缩链路的一部分，因此失败必须显式暴露。
...
- 已摘要覆盖的旧历史不会再重复参与后续 `soft_limit` 判断，除非快照失效或覆盖边界找不到。
...
- 当前 system 消息依然全部固定保留；如果固定上下文本身已经很大，即使旧历史被折叠，也可能仍然频繁逼近 `soft_limit`。
