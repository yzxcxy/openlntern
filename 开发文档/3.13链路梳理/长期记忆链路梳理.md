...
- 默认每 3 秒轮询一次
- 每次最多处理 10 个线程
- Redis 未配置时不启动 worker
- 启动时会把历史遗留的 `status=syncing` 回收为 `pending`
多实例下的互斥依赖 Redis 线程级锁，不再依赖 MySQL `status=syncing` 抢占。
### worker 如何挑任务

`ProcessPendingMemorySyncStates(...)` 会：
1. 查出 `status in (pending, failed)` 且 `next_attempt_at <= now` 的线程
2. 先尝试获取 Redis 锁 `openintern:memory-sync:lock:{thread_id}`
