...
type Message struct {
	...
	ThreadID string `gorm:"index;index:idx_thread_sequence,priority:1;not null;size:64" json:"thread_id"`
	RunID    string `gorm:"index;not null;size:64" json:"run_id"`
	// Sequence 保存线程内稳定递增的消息顺序，用于历史回放，避免同一时间戳下顺序漂移。
	Sequence int64 `gorm:"index:idx_thread_sequence,priority:2;not null;default:0" json:"sequence"`
