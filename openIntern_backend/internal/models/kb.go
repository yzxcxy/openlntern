...
type KnowledgeBase struct {
	...
	OpenVikingURI string `gorm:"column:openviking_uri;size:512" json:"openviking_uri"` // OpenViking中该知识库实际导入根目录的URI，用于检索和状态探测
	LocalPath     string `gorm:"column:local_path;size:512" json:"local_path"`         // 本地MinIO存储路径前缀，用于预览
	// 索引状态字段
	IndexTaskID string         `gorm:"column:index_task_id;size:64" json:"index_task_id"`                 // 兼容保留字段；资源导入不再依赖task_id轮询
