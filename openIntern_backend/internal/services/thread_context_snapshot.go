...
func (s *ThreadContextSnapshotService) GetLatestByThreadID(threadID string) (*models.ThreadContextSnapshot, error) {
	if strings.TrimSpace(threadID) == "" {
		return nil, errors.New("thread_id is required")
	}
	return dao.ThreadContextSnapshot.GetLatestByThreadID(strings.TrimSpace(threadID))
}
