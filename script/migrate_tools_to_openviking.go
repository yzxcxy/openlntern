...
// main 读取参数并执行 MySQL -> OpenViking 工具迁移。
func main() {
	options := parseFlags()
	if err := runMigration(options); err != nil {
		log.Printf("migrate tools to openviking failed: %v", err)
		os.Exit(1)
	}
}
...
func parseFlags() migratorOptions {
	...
	flag.StringVar(&pluginIDsArg, "plugin_ids", "", "comma-separated plugin ids, empty means all plugins")
...
func runMigration(options migratorOptions) error {
	...
	if len(pluginIDs) == 0 {
		log.Printf("no plugins found, skip migration")
		return nil
	}
	...
	for _, pluginID := range pluginIDs {
		...
		if getErr == nil {
			if isMCPRuntime(pluginRecord.RuntimeType) {
				skipped++
				log.Printf("skip mcp plugin migration plugin_id=%s runtime_type=%s", pluginID, pluginRecord.RuntimeType)
				continue
			}
		} else if !errors.Is(getErr, dao.ErrPluginNotFound) {
		...
		if statErr != nil {
			...
			log.Printf(
				"plugin tool snapshot plugin_id=%s total=%d enabled=%d disabled=%d unique_names=%d duplicate_name_entries=%d",
				pluginID,
				stats.Total,
				stats.Enabled,
				stats.Disabled,
		...
		if err := pluginsvc.Plugin.SyncPluginToolsToOpenViking(ctx, pluginID); err != nil {
			log.Printf("sync plugin tools failed plugin_id=%s err=%v", pluginID, err)
			failed = append(failed, pluginID)
			continue
		}
		success++
		log.Printf("sync plugin tools succeeded plugin_id=%s", pluginID)
	}
...
// loadConfigWithFallback 加载配置文件，并支持从 script 目录执行时回退到上级目录。
func loadConfigWithFallback(configPath string) (*config.Config, string, error) {
	...
	if !filepath.IsAbs(configPath) {
		candidates = append(candidates, filepath.Join("..", configPath))
	}
