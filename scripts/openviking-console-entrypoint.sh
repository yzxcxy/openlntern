...
# Allow the OpenViking HTTP server to bind on an externally reachable host in local development.
SERVER_HOST="${OPENVIKING_SERVER_HOST:-0.0.0.0}"
SERVER_PORT="${OPENVIKING_SERVER_PORT:-1933}"
...
SERVER_CMD="/app/.venv/bin/openviking-server --host ${SERVER_HOST} --port ${SERVER_PORT}"
if [ "${WITH_BOT}" = "1" ]; then
    # Keep bot proxy behavior unchanged while exposing the HTTP server to the host.
    sh -lc "${SERVER_CMD} --with-bot" &
else
    sh -lc "${SERVER_CMD}" &
fi
SERVER_PID=$!
